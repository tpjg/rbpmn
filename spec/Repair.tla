...
(* that back by restoring `active` — Repair_ThawIsReachable.cfg reaches a   *)
(* stranded sibling completing after one — and must not cost the lease any  *)
...
\* active, the sibling is claimable or completable again. It restates the
\* lease's own guards — on an active instance an available item is anyone's
\* to complete and a locked one its holder's — and holds trivially while the
\* instance is frozen, so it cannot tell a thaw from none:
\* Repair_ThawIsReachable.cfg is what pins that a repair thaws. The way a
\* thaw really strands a worker is latency: a lease that lapsed during the
\* freeze, and nobody woken for it. No model here sees a poll interval;
\* `resume_after_freeze`'s wake answers it.
...
\* the thaw's case, reached. It is the one check here that a repair thaws
\* at all: with RepairLands leaving the instance frozen, it finds nothing.
