...
// planes.
//
// Both documents need it, for the same reason from opposite ends: the editor
// opens hand-written .bpmn files that have no diagram, and the inspector
// renders whatever XML was deployed — which, in a project whose own corpus is
// hand-written, is not guaranteed to carry one either. A debug view that
// shows a blank canvas because DI is missing is a debug view that fails
// exactly when someone is already in trouble.
//
// Kept in sync with playground/src/layout.js by hand; the two are separate
// bundles with no shared package.
