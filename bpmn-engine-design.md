...
| Message boundary events | Example: payment-during-contest: a `PAID` correlated to a ticket parked at a user task is refused with `NoSubscription` today | Medium | **Slice 1 shipped** — the interrupting form on all four hosts, with `Lost { state }` pulled forward so a frontend can tell *withdrawn* from *reassigned*; [docs/design/boundary-messages.md](docs/design/boundary-messages.md) is the record (it also closed a dead arm lint accepted — a timer boundary on a business-rule task — and a loader that resolved a token's subscription by token alone). Slices 2–3 (non-interrupting, `timeCycle`) follow |
