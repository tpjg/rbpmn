...
| Message boundary events | Example: payment-during-contest: a `PAID` correlated to a ticket parked at a user task is refused with `NoSubscription` today | Medium | **Design round done** — [docs/design/boundary-messages.md](docs/design/boundary-messages.md). Slice 1 is the interrupting form on all four hosts; also names a dead arm lint accepts today (a timer boundary on a business-rule task) and a loader gap (two subscriptions on one token) |
